...
- 任务管理：删除 / 取消 / 重命名、失败重跑、**从已有产物继续重试**、仅重试翻译
- 产物下载、自定义单个 Job 输出目录、打开输出目录、yt-dlp / WhisperKit 日志查看
...
Job 工作目录默认位于：`~/Library/Application Support/CaptionFlow/Data/jobs/<job_id>/`（可用 `TM_DATA_DIR` 覆盖）。

每个 Job 可选填写：

- **任务名称**：用于历史记录显示；不填时使用上传文件名、URL 或 yt-dlp 下载标题。
- **输出目录**：最终 SRT / TXT / Markdown / JSON 会复制到该目录；不填则留在默认 Job 工作目录。输入文件、中间缓存、日志仍保存在内部工作目录。

失败重试会尽量复用已有产物：已下载的视频、已合成音轨、已转写字幕、已翻译字幕会被检测并跳过，避免从头重新跑。
...
**WhisperKit runtime**（`runtime/whisperkit/`，已 gitignore）需在克隆后自行准备，见下方第 1 步。  
**媒体工具**（`runtime/media/`，已 gitignore）用于 ffmpeg / ffprobe / yt-dlp，macOS 打包时会 bundle 进 App。
...
### 1b. 准备媒体工具（ffmpeg / yt-dlp）

macOS 打包前运行（Apple Silicon 需已安装 `brew install ffmpeg`）：

```bash
./scripts/setup_media_tools.sh --ensure
```

产物位于 `runtime/media/bin/`（Apple Silicon 还会把 ffmpeg 依赖库放到 `runtime/media/lib/`）。Web 开发模式也可继续用系统 PATH 中的 ffmpeg / yt-dlp，或在 `.env` 里指向上述路径。
...
- 已准备好的 `runtime/whisperkit/` 与 `runtime/media/`（`tauri:build` 前会自动运行 `setup_media_tools.sh --ensure`）
...
| 变量 | 说明 | 默认 |
|------|------|------|
| `TM_FFMPEG_EXECUTABLE` | ffmpeg 可执行文件 | `ffmpeg` |
| `TM_FFPROBE_EXECUTABLE` | ffprobe 可执行文件 | `ffprobe` |
| `TM_YTDLP_EXECUTABLE` | yt-dlp 可执行文件 | `yt-dlp` |

macOS App 会从 bundle 内 `runtime/media/bin/` 注入以上三个绝对路径。
